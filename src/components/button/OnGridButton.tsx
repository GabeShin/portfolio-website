...
export default function OnGridButton({ text, onClick }: PropsType) {
  return (
    <button
      className="clickable-button group relative h-10 w-10 rounded-full border-2 border-border transition-all duration-300 hover:w-36 hover:ring-4 hover:ring-white hover:ring-opacity-50"
      onClick={onClick}
    >
